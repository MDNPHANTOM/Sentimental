        ...
        <div class="main-info2">
            ...
            @foreach ($postreports as $postreport)
                <div class="main-info">
                    <div class="main-frame427320725">
                        <div class="main-post-info">
                            <a href="{{ route('admin.reported_posts', $postreport->user_id) }}" class="main-text">{{ $postreport->user->name }}</a>
                            <span class="main-text02">---</span>
                            <span class="main-text04">{{$postreport->created_at->format('Y-m-d H:i:s')}}</span>
                            @if (Auth::user()->isAdmin == 1)
                            <div class="rem-element">
                                <form action="{{ route('reports.delete_post_report', ['report' => $postreport->id]) }}?{{ time() }}" method="POST">
                                    @csrf
                                    @method('DELETE')
